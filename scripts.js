...
function checkExistingReward(providedEmail) {
  ...
  if (claimedData[normalizedEmail]) {
    ...
    if (currentTime - lastClaimTime < thirtyDays) {
      ...
      addMessage(
        "You have already claimed a reward. Please check your previous email from us to see when you're eligible to play again", "bot"
      );
    ...
    reader.onload = async function () {
      ...
      try {
        const response = await fetch(
          "https://script.google.com/macros/s/AKfycbz46grD077IpLq-S8EzSthQSrOhCsteB-x2iZb1_JpuIGCJ8Oud5UGH80kXItKrpKyY/exec ",
